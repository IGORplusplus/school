...
int Cell::garbageCount = 0;
int Cell::garbageCollected = 0;
...
Cell* Cell::firstCellofType(vector<vector<Cell>> adjCells, char type){
    if(GetAdjTL() != nullptr){
	if(GetAdjTL()->GetType() == type){
	    return GetAdjTL();
	}
    }
    else if(GetAdjT() != nullptr){
	if(GetAdjT()->GetType() == type){
	    return GetAdjT();
	}
    }
    else if(GetAdjTR() != nullptr){
	if(GetAdjTR()->GetType() == type){
	    return GetAdjTR();
	}
    }
    else if(GetAdjL() != nullptr){
	if(GetAdjL()->GetType() == type){
	    return GetAdjL();
	}
    }   
    else if(GetAdjR() != nullptr){
	if(GetAdjR()->GetType() == type){
	    return GetAdjR();
	}
    }
    else if(GetAdjBL() != nullptr){
	if(GetAdjBL()->GetType() == type){
	    return GetAdjBL();
	}
    }
    else if(GetAdjB() != nullptr){
	if(GetAdjB()->GetType() == type){
	    return GetAdjB();
	}
    }
    else if(GetAdjBR() != nullptr){
	if(GetAdjBR()->GetType() == type){
	    return GetAdjBR();
	}
    }
